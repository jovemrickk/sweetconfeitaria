...
import {
  ...
  ChevronRight, AlertTriangle, CircleDollarSign, RotateCcw, Menu
} from 'lucide-react';
...
export default function AppShell() {
  const { data, setData, loaded, reset, exportJson, importJson } = useAppData();
  const [tab, setTab] = useState<Tab>('dashboard');
  const [mobileNav, setMobileNav] = useState(false);

  if (!loaded) return <div className="loading"><div className="spinner" />Carregando Sweet Dreams...</div>;
    ...
    <div className="app">
      <aside className={`sidebar ${mobileNav ? 'open' : ''}`}>
        <div className="brand">
          ...
          <div><strong>Sweet Dreams</strong><span>gestão da confeitaria</span></div>
        ...
        <div className="sidebarNote">
          ...
          <div><strong>Modo local</strong><small>Seus dados ficam neste navegador.</small></div>
        </div>
      </aside>
